...
const CIRCUIT_BREAKER_THRESHOLD = 3; // consecutive fails
...
// ─── SourcePipeline Class ──────────────────────────────────────────────────
class SourcePipeline {
  constructor(config = {}) {
    this.config = config;
    this.instances = {}; // { source: { url: { health, latency, errors, lastCheck, circuitOpenUntil } } }
  ...
  _initDefaultSources() {
    const defaults = {
      invidious: ['https://invidious.snopyta.org', 'https://vid.puffyan.us', 'https://yewtu.be'],
      piped: ['https://piped.kavin.rocks', 'https://piped.privacydev.net'],
    ...
    for (const [source, urls] of Object.entries(defaults)) {
      ...
      for (const url of urls) {
        this.instances[source][url] = {
          health: 1.0, latency: 0, errors: 0,
          lastCheck: 0, circuitOpenUntil: 0
        };
      }
    }
  }
  ...
  getHealthyInstances(name) {
    ...
    return Object.entries(this.instances[name])
      .filter(([url, stats]) => {
        if (stats.circuitOpenUntil > now) return false;
        return stats.health >= HEALTH_THRESHOLDS.degraded;
      })
  ...
  async testAll() {
    ...
    for (const [source, instances] of Object.entries(this.instances)) {
      ...
      for (const [url, stats] of Object.entries(instances)) {
        ...
        try {
          const r = await this._proxiedFetch(`${url}/api/v1/trending/music?type=video&sort=weekly`, {
            method: 'GET', timeout: 5000
          });
          const latency = Date.now() - start;
          const ok = r.ok || r.status === 200;
          this.markInstance(url, ok, latency);
          results[source][url] = { status: ok ? 'ok' : 'failed', latency, health: this.instances[source][url].health };
        } catch (e) {
          const latency = Date.now() - start;
  ...
  markInstance(url, ok, latency = 0) {
    for (const [source, instances] of Object.entries(this.instances)) {
      if (instances[url]) {
        ...
        stats.health = (HEALTH_WEIGHTS.latency * latencyScore +
                       HEALTH_WEIGHTS.errorRate * (1 - errorRate) +
                       HEALTH_WEIGHTS.age * (1 - ageScore));
        return;
      }
    }
  }
  ...
  getHealthSummary() {
    ...
    for (const [source, instances] of Object.entries(this.instances)) {
      ...
      for (const [url, stats] of Object.entries(instances)) {
        if (stats.circuitOpenUntil > now) {
          down++;
        } else if (stats.health >= HEALTH_THRESHOLDS.healthy) {
          healthy++;
        } else if (stats.health >= HEALTH_THRESHOLDS.degraded) {
          degraded++;
        } else {
          down++;
        }
  ...
  async _proxiedFetch(url, options = {}) {
    const proxy = this.getNextProxy();
    if (proxy && !url.startsWith('http://metube')) {
      // For non-MeTube URLs, inject proxy
      const proxyUrl = new URL(proxy);
      options.agent = new (require('https').Agent)({ proxy: { host: proxyUrl.hostname, port: proxyUrl.port } });
    }
...
const SEARCH_HANDLERS = {
  invidious: async (query, cfg) => {
    const instances = cfg.sourcePipeline.getHealthyInstances('invidious');
      ...
      try {
        ...
        if (r.ok) {
          ...
          return data.map(t => ({
            ...
            title: t.title,
            artist: t.author,
            duration: t.lengthSeconds,
            url: `${base}/watch?v=${t.videoId}`,
  ...
  piped: async (query, cfg) => {
    const instances = cfg.sourcePipeline.getHealthyInstances('piped');
    for (const base of instances) {
      try {
  ...
  dab: async (query, cfg) => {
    const instances = cfg.sourcePipeline.getHealthyInstances('dab');
    for (const base of instances) {
      try {
...
function invidiousInstances() {
  return ['https://invidious.snopyta.org', 'https://vid.puffyan.us', 'https://yewtu.be'];
}

async function proxiedFetch(url, options = {}) {
  // This will be replaced by the pipeline's proxiedFetch when initialized
